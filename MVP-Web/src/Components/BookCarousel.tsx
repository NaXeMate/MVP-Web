    ...
    <section className="carousel-container">
      <article className="carousel-wrapper">
        ...
        <div className="books-scene">
          <div
            className="books-cylinder"
            style={{ transform: `rotateY(${-rotacionActual}deg)` }}
          >
            {libros.map((libros, index) => {
              ...
              return (
                <div
                  key={libros.id || index}
                  className={`book-card ${esActivo ? "book-active" : ""}`}
                  style={{
                    transform: `rotateY(${angulo}deg) translateZ(${radio}px) scale(${escala})`,
                  }}
                  onClick={() => handleGoToBook(index)}
                >
